...
mx_query(int fd, int b1, int *res)
{
    ...
    if (res[0] != first_byte || res[1] != 0x81 || res[2] != b1)
    {
      printf("bad answer (%02x %02x %02x %02x %02x %02x )\n", 
...
trouble_shooting(void)
{
    ...
    fatal(
"Hiddev kernel driver not found.  Check with 'dmesg | grep hiddev'\n"
...
main(int argc, char **argv)
{
    ...
    if (argc < 2 || (argc > 1 && (streq(argv[1], "-h") || streq(argv[1], "--help"))))
      usage();
